...
export default function Checkout(props) {
  ...
  useEffect(async () => {
    const action = getSeatBookingListApi(id);
    dispatch(action);
  }, []);
    ...
    return seatSelection.map((item, index) => {
      return (
        ...
        >
          ...
          <p className="w-1/3 mb-0">Giá vé: {item["giaVe"].toLocaleString()} đ</p>
          <p className="w-1/3 pl-3 mb-0">Loại ghế {renderSeatType(item.loaiGhe)}</p>
        </div>
      );
    });
  };
      ...
      <div className="checkout-infor p-5 flex justify-between">
        ...
        <div className="user-infor w-2/5 text-white flex border-gray-400">
          <div className="content px-5 flex-row self-center">
            ...
            <p className="my-auto text-base">Địa chỉ: 239 Phan Xích Long, Phú Nhuận</p>
          </div>
        </div>
      </div>
